...
Bayesian methods are also well-suited to accommodate the small population sizes that  are often at high risk of extinction and thus are often a focus of conservation \citep{stinchcombe2002influence}. Because Bayesian methods do not rely on asymptotic behavior (as frequentist statistics do \citep{mcneish2016using}), they are better able to accommodate small sample sizes. % emw15Nov2025: Given the audience I would avoid 'asymptotic' -- maybe just delete the sentence?
