...
\section*{Abstract} 
MAke more focused on why we should integrate Bayesian more into current frameworks/resolutions etc. including an angle towards better adaptive management and forecasting with climate change.
...
\item We envision a future in which conservation and ecology students (undegraduate and graduate levels) recieve statistical training to provide strong foundations in Bayesian statistics. Many introductory statistics classes focus on t-tests and linear regressions, which are often not appropriate for ecological datasets. It doesn't have to be this way! 
