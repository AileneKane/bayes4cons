...
At the foundation of this statistical approach is the use of Bayes' Theorem (eqn 1). This theorem consists of three components: a prior distribution (A) as defined using data from previous experiments, expert opinions, or literature, and a likelihood function ($f(B|A)$), which are combined to estimate the probability distributions for each parameter (\emph{P}(A|B)). \\
\begin{align}
