...
\subsection*{Bayesian frameworks for integrating multiple data sources}
...
\subsection*{Moving beyond null hypothesis testing: comparing alternatives for adaptive management}
...
\subsection*{Bayesian approaches for quantifying and propagating uncertainty} 
...
\par  Though Bayesian methods are increasing, the are not standard, widely used s approaches that are well-integrated into global systems of conservation and climate science, policy and practice (e.g., IPCC, IUCN). For example, IPCC-described methods for uncertainty propogation do not include Bayesian approaches (REF) and 
...
\section* {Figures}
 \begin{figure}[h]
\centering
 \includegraphics{../figs/BayesianWorkflowfromGelman2020.jpg}
  \includegraphics{../figs/The-Open-Standards-adaptive-management-cycle-from-wwwconservationmeasuresorg.png}
 \caption{\textbf{Do we want to include something like this Bayesian workflow? Also, it seems aligned with adaptive management cycles, which are comnonly used in conservation biology. We could consider a 2-paneled figure with sometlike these? I would love to discuss this as a group, if people think it would be useful}} 
 \label{fig:workflow}
 \end{figure}
