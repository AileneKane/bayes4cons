...
To simulate data that fits our needs, we use a simple linear function and add noise. We want the population data to exhibit either an increasing or decreasing trend, but some trends are not statistically significant (with a p-value greater than 0.05). After
...
We want to search for cases were:
...
plot(1, type = "n", xlab = "Change in abundance", ylab = "Frequency", xlim = c(-2500, 2500), ylim = c(0, 1500), 
     cex.lab = 1.5, frame.plot = FALSE)
...
for(i in 1:length(unique(output$pop))) {
  ...
  arrows(x0 = (quantile(post$b[,i], 0.05)), y0 = 1300, x1 = (quantile(post$b[,i], 0.95)), y1 = 1300, 
         code = 3, length = 0, col = colors[i], lwd = 2)
}
```
