...
To demonstrate the ease with which we can use Bayesian methods to include biological information to help fit complex population models, we will explore a relatively simple state-space models for population described in described in Auger-Méthé et al. (2021). This model is based on one of the models Dennis and Taper (1994) and Jamieson and Brooks (2004) have proposed to include density denpendece, biological stochasticiy, and measurement error in a single model. 
