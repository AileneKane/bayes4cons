...
This species has a long life span (20+ years) and creates long term pairs that can produce maximum 2 offspring when in good conditions, and therefore could be able to almost double in size every year. However, here we simulate a situation where the conservation area does not allow the animal to fulfill its full growth and we simul by setting $\beta_0 = \text{log}(1.3)$. We assumed when doing the yearly transects we can miss or double count individuals and simulate this by setting $\sigma_o = 10$. We know that for most population of this species, the birth rate, which is the main source of biological stochasticity, vary by less than 5%.
